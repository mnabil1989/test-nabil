...
class AppointmentController extends \TCG\Voyager\Http\Controllers\VoyagerBaseController
{
    ...
    public function index(Request $request)
    {
        if($request->doctor_id){
            $model = Appointment::with('doctorId')->where('doctor_id',$request->doctor_id)->with('userId')->get();
             
        }elseif($request->speciality){
            
            $model = Appointment::with(['doctorId' => function ($query) {
                $query->where('speciality', 'like', $_REQUEST['speciality']);
            }])->with('userId')->get();

            //convert the collection to an array of objects because a js at  view  waiting for an array of objects
            ...
            $model=$modelContaner;
                 ...
                 $model = Appointment::with('doctorId')->with('userId')->get();
             }
        return view('vendor.voyager.appointments.browse', compact(
            'model'
        ));
    }
