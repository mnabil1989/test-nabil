    ...
    function clinics(Request $request)
    {
        ...
        if($request->clinics || $request->specialities || $request->gender || $request->all_doctors)
        {
            if($request->clinics)
            {
                ...
                foreach ($all as $clinic_id)
                {
                    $clinics = Clinic::where('id', $clinic_id)->get();
            ...
            if($request->specialities)
            {
                ...
                foreach ($all as $speciality_id)
                {
                    $specialities = Speciality::where('id', $speciality_id)->get();
            ...
            {
                $doctors = Doctor::where('status', 1)->with('working_hours')->get();
                foreach ($doctors as $doctor)
                    $data[intval((integer)$doctor->id)] = $doctor;
            }
...
//                ->orWhere('translations.value', 'like', '%' . request()->query_search . '%')
        ...
        $openGraphFields =SiteSetting::where('key',"like","%clinics.facebook.%")->get();
        $open_graph=[];
        foreach($openGraphFields as $oGField){
            if(strstr($oGField,"image") ){
                ...
                $open_graph['open_graph_'.explode("clinics.facebook.",$oGField->key)[1]]=$value;
        }

        $twitterCardFields =SiteSetting::where('key',"like","%clinics.twitter.%")->get();
        $twitter_card=[];
        foreach($twitterCardFields as $cardField){
            if(strstr($cardField,"image") ){
                ...
                $twitter_card['twitter_card_'.explode("clinics.twitter.",$cardField->key)[1]]= $value;
        }

        $metaTags =SiteSetting::where('key',"like","%clinics.meta.%")->get();
        $meta_tags=[];
        foreach($metaTags as $metaTag){
                $meta_tags['meta_'.explode("clinics.meta.",$metaTag->key)[1]]=$metaTag->value;
        }
        if ($request->input('lang') == 'en') {
            $clinics = ClinicResource::collection($clinics);
            return response()->json(compact(['clinics','meta_tags','twitter_card','open_graph']), 200);
    ...
    function clinic($slug,Request $request)
    {
        ...
        $clinic_ar = [
            ...
            "description" => preg_replace('/(<[^>]+) style=".*?"/i', '$1', $clinicWithSlug->description),
            ...
            "photo" => url('/storage/'. str_replace("\\", '/', $clinicWithSlug->photo)),
            "image_alt" => $clinicWithSlug->image_alt,
            "image_title" => $clinicWithSlug->image_title,
            "meta_keywords" => $clinicWithSlug->meta_keywords,
            "meta_description" => $clinicWithSlug->meta_description,
            "facebook_title"=> $clinicWithSlug->facebook_title,
            "facebook_description"=> $clinicWithSlug->facebook_description,
            "facebook_image"=> str_replace("\\", "/",url('storage/'.$clinicWithSlug->facebook_image) ),
            "twitter_title"=> $clinicWithSlug->twitter_title,
            "twitter_description"=> $clinicWithSlug->twitter_description,
            "twitter_image"=> str_replace("\\", "/",url('storage/'.$clinicWithSlug->twitter_image) )
