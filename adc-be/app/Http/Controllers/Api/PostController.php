...
class PostController extends Controller
{
    ...
    function all_posts(Request $request)
    {
        ...
        if($request->input('lang')=='en'){                   
            $posts_arr=Post::where(['status'=>'PUBLISHED'])->with('category')->with('tag')->with('translations')->OrderBy("order","asc")->paginate($pagination)->appends('lang',$request->input('lang'));              
            $data=PostDataResource::collection($posts_arr);                
        }else{
            $posts_arr=Post::where(['status'=>'PUBLISHED'])->with('category')->with('tag')->with('translations')->OrderBy("order","asc")->paginate($pagination)->appends('lang',$request->input('lang'));
            $data=PostDataArResource::collection($posts_arr);           
        }                       
    return response()->json(compact(['data']), 200);
    }   
    function most_viewed(Request $request)
    {
        if(empty($request->input('lang'))){
            return response()->json("please enter language you want", 200);
        }
        if($request->input('lang')=='en'){                
            $posts_arr=Post::where(['status'=>'PUBLISHED','featured'=>1])->with('translations')->OrderBy("order","asc")->paginate(8)->appends('lang',$request->input('lang'));              
            $data=PostDataResource::collection($posts_arr);                
        }else{
            $posts_arr=Post::where(['status'=>'PUBLISHED','featured'=>1])->with('translations')->paginate(3)->appends('lang',$request->input('lang'));
            $data=PostDataArResource::collection($posts_arr);           
        }                       
    return response()->json(compact(['data']), 200);
    }  
    ...
    function post($slug, Request $request)
    {
        ...
        $post_ar = [
            ...
            "body" => $this->strip_word_html($post->body),
            "body" => $post->body,
            "slug" => $post->slug,
...
//______________________End____Translation Category__________________________________________
        $post_en= [
            ...
            "image_title" => $post->image_title,
            "excerpt" => $excerpt_en,        
            "body" => $this->strip_word_html($body_en),
            "slug" => $slug_en,
...
//------------------------End-------Post En--------------------------------------------
    ...
    {
        if ($request->input('lang') == 'en') {
            $data = CategoryResource::collection(Category::with('posts')->with('translations')->get());
            return response()->json(compact(['data']), 200);
        } elseif ($request->input('lang') == 'ar') {
            $data = CategoryArResource::collection(Category::with('posts')->with('translations')->get());
            return response()->json(compact(['data']), 200);
        } else {
            return response()->json("please enter language you want", 200);
        }
