        ...
        <ul class="nav navbar-nav @if (__('voyager::generic.is_rtl') == 'true') navbar-left @else navbar-right @endif">
            <li class="dropdown profile">
                <a href="#" class="dropdown-toggle text-right" data-toggle="dropdown" role="button"
                   aria-expanded="false"><img src="https://img.pngio.com/english-to-arabic-icon-png-50-px-free-english-png-1600_1600.png"> <span
                            class="caret"></span></a>
                <ul class="dropdown-menu dropdown-menu-animated">

                    <?php $nav_items = config('voyager.dashboard.navbar_languages'); ?>
                    ...
                    <li {!! isset($item['classes']) && !empty($item['classes']) ? 'class="'.$item['classes'].'"' : '' !!}>
                        ...
                        <a href="{{ isset($item['route']) && Route::has($item['route']) ? route($item['route']) : (isset($item['route']) ? $item['route'] : '#') }}" {!! isset($item['target_blank']) && $item['target_blank'] ? 'target="_blank"' : '' !!}>
                            @if(isset($item['icon_class']) && !empty($item['icon_class']))
                                <img src="{!! $item['icon_class'] !!}"width="20px"height="20px">
                            @endif
                            {{__($name)}}
                        </a>
                        @endif
