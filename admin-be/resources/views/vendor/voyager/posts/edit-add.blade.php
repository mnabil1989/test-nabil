                ...
                <div class="col-md-8">
                    ...
                    <div class="panel">
                        ...
                        <div class="panel-body">
                            ...
                            <textarea class="form-control"
                                      name="excerpt" id="excerpt">{{ $dataTypeContent->excerpt ?? '' }}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="panel">
                        ...
                        <div class="panel-body">
                            @php
                                $dataTypeRows = $dataType->{(isset($dataTypeContent->id) ? 'editRows' : 'addRows' )};
                                $exclude = ['title', 'body', 'excerpt', 'slug', 'status', 'category_id', 'author_id', 'featured', 'image', 'meta_description', 'meta_keywords', 'seo_title','post_belongstomany_tag_relationship'];
                            @endphp
                            ...
                            @foreach($dataTypeRows as $row)
                                @if(!in_array($row->field, $exclude))
                                    ...
                                    @else
                                        <div
                                            class="form-group @if($row->type == 'hidden') hidden @endif @if(isset($display_options->width)){{ 'col-md-' . $display_options->width }}@endif" @if(isset($display_options->id)){{ "id=$display_options->id" }}@endif>
                                            {{ $row->slugify }}
                        ...
                        <div class="panel-body">
                            ...
                            @foreach($dataTypeRows as $row)
                                @if(!in_array($row->field, $exclude))
                                    ...
                                    @else
                                        <div
                                            class="form-group @if(isset($display_options->width)){{ 'col-md-' . $display_options->width }}@endif" @if(isset($display_options->id)){{ "id=$display_options->id" }}@endif>
                                            <label for="name">{{ $row->getTranslatedAttribute('display_name') }}</label>
                                            @include('voyager::formfields.relationship', ['options' => $row->details])
                                        </div>
                                    @endif
                        ...
                        <div class="panel-body">
                            <div class="form-group">
                                ...
                                <textarea class="form-control" name="meta_description"
                                          id="meta_description">{{ $dataTypeContent->meta_description ?? '' }}</textarea>
                            </div>
                            <div class="form-group">
                                ...
                                <textarea class="form-control" name="meta_keywords"
                                          id="meta_keywords">{{ $dataTypeContent->meta_keywords ?? '' }}</textarea>
                            </div>
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" name="seo_title" id="seo_title"
                                       placeholder="SEO Title" value="{{ $dataTypeContent->seo_title ?? '' }}">
                            </div>
                        </div>
                    </div>
                </div>
...
@section('javascript')
    <script>
        function seoData() {
            document.cookie = 'seokeyword=;expires = Thu, 01 Jan 1970 00:00:00 GMT';
            document.cookie = 'seotitle=;expires = Thu, 01 Jan 1970 00:00:00 GMT';
            document.cookie = 'seometadescription=;expires = Thu, 01 Jan 1970 00:00:00 GMT';
            document.cookie = 'seodescription=;expires = Thu, 01 Jan 1970 00:00:00 GMT';
            document.cookie = 'seourl=;expires = Thu, 01 Jan 1970 00:00:00 GMT';
            document.cookie = 'seoen_locale=;expires = Thu, 01 Jan 1970 00:00:00 GMT';
            document.cookie = 'seopermalink=;expires = Thu, 01 Jan 1970 00:00:00 GMT';
            localStorage.removeItem('seotext');
//             var iframe = document.getElementById('iframeId');
// //             var innerDoc = iframe.innerHTML.document.getElementById("richtextbody");
// // console.log(innerDoc);
            let title = document.getElementById("title").value;
            let excerpt = document.getElementById("excerpt").value;
            let body = document.querySelector("#richtextbody_ifr").contentWindow.document.querySelector("#tinymce").innerHTML;
            ...
            let  seourl= '{{ url('/') }}';
            let seopermalink = '{{ url('/') }}' + '/' + slug;
            let seoen_locale = 'ar';
            document.cookie = 'seokeyword=' + meta_keywords;
            document.cookie = 'seotitle=' + title;
            document.cookie = 'seometadescription=' + excerpt;
            document.cookie = 'seodescription=' + meta_description+'0';
            document.cookie = 'seourl=' + slug;
            document.cookie = 'seoen_locale=' + seoen_locale;
            document.cookie = 'seopermalink=' + seopermalink;
            localStorage.setItem("seotext", body);
        };
    </script>
    <script>
