...
@section('page_header')
    <h1 class="page-title">
        <i class="voyager-megaphone"></i> Yaost
    </h1>
@stop
...
@section('content')
    ...
    <div class="page-content settings container-fluid">
        <form action="{{ route('voyager.sitesettings.update') }}" method="POST" enctype="multipart/form-data">
            {{ method_field("PUT") }}
            {{ csrf_field() }}
                                ...
                                @foreach($group_settings as $setting)
                                    <div class="panel-heading">
                                        ...
                                        <div class="panel-actions">
                                            <a href="{{ route('voyager.sitesettings.move_up', $setting->id) }}">
                                                <i class="sort-icons voyager-sort-asc"></i>
                                            </a>
                                            <a href="{{ route('voyager.sitesettings.move_down', $setting->id) }}">
                                            ...
                                            @elseif($setting->type == "image" || $setting->type == "file")
                                                @if(isset( $setting->value ) && !empty( $setting->value ) && Storage::disk(config('voyager.storage.disk'))->exists($setting->value))
                                                    <div class="img_settings_container">
                                                        <a href="{{ route('voyager.sitesettings.delete_value', $setting->id) }}" class="voyager-x delete_value"></a>
        ...
        @can('add', Voyager::model('Setting'))
            <div class="panel" style="margin-top:10px;">
                ...
                <div class="panel-body">
                    <form action="{{ route('voyager.sitesettings.store') }}" method="POST">
                        {{ csrf_field() }}
                        <input type="hidden" name="setting_tab" class="setting_tab" value="{{ $active }}" />
                        <div class="col-md-3">
    ...
    <script>
        $('document').ready(function () {
            ...
            $('.panel-actions .voyager-trash').click(function () {
                ...
                $('#delete_form')[0].action = '{{ route('voyager.sitesettings.delete', [ 'id' => '__id' ]) }}'.replace('__id', $(this).data('id'));
