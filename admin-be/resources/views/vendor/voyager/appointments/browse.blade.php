...
@section('content')
    ...
    <div class="page-content browse container-fluid">
        ...
        <div class="row">
            <div class="col-md-12">
            <!-- ---------------------- -->
            <select id="selectSpecialityBox" name="specialityId"  class="fc-today-button fc-button fc-button-primary" onchange="filterSpecialityFunc()">
                <option value="">Select speciality...</option>
              
                @php ($specialityList = [])
                
                @foreach($model as $doctorSpeciality)
                
                
                    @if(!in_array($doctorSpeciality->doctorId->speciality,$specialityList))
                <option value="{{$doctorSpeciality->doctorId->speciality}}">{{$doctorSpeciality->doctorId->speciality}}</option>
                     @php ($specialityList[$doctorSpeciality->doctorId->id] = $doctorSpeciality->doctorId->speciality)
            ...
            <!-- ---------------------- -->
            <select id="selectDoctorBox" name="doctorId"  class="fc-today-button fc-button fc-button-primary" onchange="filterDocotrFunc()">
                <option value="">Select doctor...</option>
              
                @php ($doctorList = [])
                @foreach($model as $doctor)
                
                    @if(!in_array($doctor->doctorId->title_ar,$doctorList))
                <option value="{{$doctor->doctorId->id}}">{{$doctor->doctorId->title_en}}</option>
                     @php ($doctorList[$doctor->doctorId->id] = $doctor->doctorId->title_ar)
                    @endif
                  @endforeach
                <option value="">List All...</option>
            </select> 
...
@section('javascript')
   ...
   function filterSpecialityFunc() {
    var selectBox = document.getElementById("selectSpecialityBox");
    var selectedValue = selectBox.options[selectBox.selectedIndex].value;
    var route ;
  
    if( selectedValue !==''){
       route ="{{route(\Route::current()->getName())}}"+'?speciality='+selectedValue;
    }else{
   ...
   function filterDocotrFunc() {
    var selectBox = document.getElementById("selectDoctorBox");
    var selectedValue = selectBox.options[selectBox.selectedIndex].value;
    var route ;
  
    if( selectedValue !==''){
       route ="{{route(\Route::current()->getName())}}"+'?doctor_id='+selectedValue;
    }else{
        route ="{{route(\Route::current()->getName())}}";
    }
