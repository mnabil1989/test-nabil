                  ...
                  <ValidationObserver v-slot="{ handleSubmit }">
                    <form @submit.prevent="handleSubmit(SubmitNewUser(registerInfo))">
                      <div class="row for-Number">
                        ...
                        <ValidationProvider class="col-12" :rules="{required: true,regex: /^05([0-9]{8})$/}" v-slot="{ errors }">
...
<!--                        </div>-->
<!--                        <ValidationProvider-->
<!--                          class="col-3 col-md-3 number"-->
<!--                          :rules="{required: true,regex: /^(5|0|3|6|4|9|1|8|7)([0-9]{0})$/}"-->
<!--                          v-slot="{ errors }"-->
<!--                        >-->
<!--                          <div>-->
...
<!--                            </div>-->
<!--                          </div>-->
<!--                          <p class="phone-ksa paragraph"></p>-->
<!--                          <p class="p-error">{{ errors[0] }}</p>-->
<!--                        </ValidationProvider>-->
<!--                        <ValidationProvider-->
<!--                          class="col-4 col-md-6 number"-->
                ...
                >
                  ...
                  >
                    <label class="payment-radio paypal-option" :class="{'pr-4':$i18n.locale=='ar'}">
                      ...
                      <span class="checkmark" :style="{right: $i18n.locale=='ar'? '0':'left'}"></span>
                      <img
                        src="../../../../../../assets/img/master-visa-mada.png"
                        alt="MasterCard"
                        class="img-fluid card-master"
                      />
                    </label>
                  </div>
                  ...
                  <!--                      &lt;!&ndash;                      <input type="radio" v-model="paymentSelected" name="radio" />&ndash;&gt;-->
                  <!--                      <span class="checkmark" :style="{right: $i18n.locale=='ar'? '0':'left'}"></span>-->
                  <!--                      {{$t('free')}}-->
                  <!--                    </label>-->
                  <!--                  </div>-->
...
export default {
  ...
  created() {
    if (!this.getDate) {
      this.$router.push(
        this.localePath(
          `/${this.$route.params.bu}/doctors/${encodeURIComponent(
  ...
  methods: {
    ...
    masterLoad() {
      if (!process.server) {
        ...
        if (document.getElementById("master") == null) {
          ...
          scriptMaster.setAttribute("type", "text/javascript");
          scriptMaster.setAttribute(
            "src",
            "https://qnbalahli.gateway.mastercard.com/checkout/version/49/checkout.js"
          );
          scriptMaster.setAttribute(
            "data-complete",
            this.localePath(`/${this.$route.params.bu}/receiptPage`)
          );
          scriptMaster.setAttribute("id", id);
          document.head.appendChild(scriptMaster);
        } else {
          var scriptMaster = document.getElementById("master");
        }
        // let oldScript = document.getElementById('meeza');
        // document.head.removeChild(oldScript);
        scriptMaster.onload = function() {
    ...
    meezaLoad() {
      if (!process.server) {
        this.loading = true;
        if (document.getElementById("meeza") == null) {
          let id = "meeza";
          var scriptMezza = document.createElement("script");
          scriptMezza.setAttribute("type", "text/javascript");
          scriptMezza.setAttribute(
            "src",
            "https://upg.egyptianbanks.com:2008/INVCHOST/js/Lightbox.js"
          );
          scriptMezza.setAttribute(
            "data-complete",
            this.localePath(`/${this.$route.params.bu}/receiptPage`)
          );
          scriptMezza.setAttribute("id", id);
          document.head.appendChild(scriptMezza);
        } else {
          var scriptMezza = document.getElementById("meeza");
        }
        // let oldScript = document.getElementById('master');
        // document.removeChild(oldScript);
        // now wait for it to load...
        var th = this;
        scriptMezza.onload = function() {
          console.log("meeza loaded");
          // loader = false
          th.loading = false;
        };
      }
    ...
    acceptPayment() {
      if (this.paymentSelected === "master" && this.accepted) {
        ...
        if (
          ...
          if (this.getAttachment.length >= 1)
            data.append("attachments[]", this.getAttachment);
          this.$axios
            .post(`/${this.$route.params.bu}-api/appointment_transit`, data, {
              headers: {
            ...
            .then(res => {
              ...
              Checkout.configure({
                ...
                order: {
                  amount: () => {
                    ...
                    return this.doctor.price;
                  },
                  currency: "EGP",
                  description: this.doctor.title,
                  id: res.data.order_id
                },
                session: {
                  id: res.data.session_idk
                },
                interaction: {
                  // operation: "PURCHASE",
                  merchant: {
                    name: this.$t("Andalusia DotCare"),
                    name: "ANDALOSIA",
                    address: {
                      line1:
                        this.$route.params.bu == "hjh"
                          ? this.$t("location")
                          : this.$t("s-location")
                    ...
                    email:
                      this.$route.params.bu == "hjh"
                        ...
                        : " Clinicssanabel@Andalusiagroup.net",
                    phone: "012-216-66-66",
                    logo: "https://dotbeit.com/_nuxt/img/503a666.png"
                  },
                  locale: this.$i18n.locale == "en" ? "en_US" : "ar_SA",
                  theme: "default",
                  displayControl: {
                    billingAddress: "HIDE",
                    customerEmail: "HIDE",
              ...
              if (this.paymentSelected == "master") {
      ...
      if (this.paymentSelected === "meeza" && this.accepted) {
        if (
          this.$auth.loggedIn &&
          this.paymentSeleced !== "" &&
          this.accepted
        ) {
          console.log("wosdfsdf");
          this.$axios
            .post(
              `/${this.$route.params.bu}-api/appointment_transit`,
              {
            ...
            .then(res => {
              ...
              this.order_id = res.data.order_id;
              localStorage.setItem("order-id", res.data.order_id);
              console.log(localStorage.getItem("order-id"));
            })
            .catch(e => {
              console.log(e);
            });
        ...
        var amount = this.doctor.price * 100; // price * 100 %
        var mID = 10310140032;
        var tID = 56037105;
        var mRN = localStorage.getItem("order-id");
        Lightbox.Checkout.configure = {
          ...
          completeCallback: data => {
            this.$router.push(this.localePath(`/receiptPage`));
