...
export default {
  ...
  methods: {
    ...
    hideRemote(e) {
      ...
      list.map(item => {
        ...
        if (!item.classList.contains("grid-small")) {
          item.classList.remove("grid-big");
          item.classList.add("grid-small");
          item.setAttribute(
            "style",
            `grid-area: span 3/span 4/${4 + 3 * (1 == 1 ? 0 : 1)}/25;
          z-index:1;width:calc(100% - 20px);height:calc(100% - 20px)`
          );
        } else {
          item.classList.remove("grid-small");

          item.classList.add("grid-big");

          item.setAttribute("style", `grid-area: span 12/span 24/13/25`);
        }
      });
    },
    ...
    async onScriptLoaded() {
      ...
      await this.$axios
        ...
        .then(res => {
          ...
          //token = '0066d078abe256647ba89ae857f3a1f8fefIACbuMgYo43mlEQstoF4SPVV/a/1lmkW3kfRLgDSfXkKzZOZPYHyjUwqIgBu2wAAHFWpXgQAAQBQA6heAwBQA6heAgBQA6heBABQA6he';
          channel = res["data"].channelName;
          //channel = 'bac9675392f4392f7cff9297748f127d';
      ...
      $.client.init($.appId, () => {
        ...
        $.subscribeStreamEvents();
        // $.client.join(this.$store.state.client.token, $.channel, $.uid, (uid) => {
        $.client.join(
          token,
          channel,
          user,
          ...
          err => {
            this.error = `<span>${err} <br> please contact support  ${
              this.$route.params.bu == "hjh" ? "012-216-66-66" : "012-216-66-66"
            }</span> `;
            jquery(".pop-up-error")
              .animate({ left: ".5%" }, 1000)
              .delay(100000)
  ...
  mounted() {
    if (!process.server) {
      var scriptAgora = document.createElement("script");
      scriptAgora.onload = this.onScriptLoaded;

      scriptAgora.setAttribute("type", "text/javascript");
      scriptAgora.setAttribute(
        "src",
        "https://cdn.jsdelivr.net/npm/agora-rtc-sdk@3.0.2/AgoraRTCSDK.min.js"
      );

      document.head.appendChild(scriptAgora);
    ...
    if ($.displayMode === "pip") {
      ...
      $.streamList.map((item, index) => {
        ...
        if (id != $.uid) {
          ...
          dom.setAttribute(
            "style",
            `grid-area: span 3/span 4/${4 + 3 * (index == 1 ? 0 : index)}/25;
          z-index:1;width:calc(100% - 20px);height:calc(100% - 20px)`
          );
        }
  ...
  beforeDestroy() {
    // console.log(this.localStream.close);

    if (this.localStream.close) {
      this.client && this.client.unpublish(this.localStream);
      this.localStream && this.localStream.close();
...
/*}*/
.grid-big {
  // grid-template-areas: span 12 / span 24/13/25;
}
.grid-small {
  // grid-template-areas: span 3 / span 4 / 4 / 25;
  // z-index: 1;
